(module
  ...
  (func $readColor (param $ch i32) (result i32)
  ...
  (func $linePower (result i32)
    ...
    (block $line_break
      (loop $line_loop
        ...
        (local.set $curr_color (call $readColor (i32.load8_u (global.get $line_index))))
        
        (i32.and
          (i32.eq (local.get $curr_color) (i32.const 0))
          (i32.gt_u (local.get $curr_number) (local.get $max_red))
        )
        if
        ...
        (i32.and
          ...
          (i32.gt_u (local.get $curr_number) (local.get $max_green))
        )
        if
        ...
        (i32.and
          ...
          (i32.gt_u (local.get $curr_number) (local.get $max_blue))
        )
        if
    ...
    (i32.mul
        (i32.mul (local.get $max_red) (local.get $max_green))
        (local.get $max_blue)
    )
  )
  ...
  (func $main (param $len i32) (result i32)
    ...
    (block $outer_break
      (loop $outer_loop
        ...
        (global.set $line_index 
            (i32.add 
                (global.get $line_index) 
                (i32.const 5)
            )
        )
        (local.set $sum_power 
